...
class QuillViewTest(TestCase):
    def test_generic_list_view(self):
        ...
        response = self.client.get(reverse('post:quill-post-list'))
        self.assertEqual(response.status_code, 200)

    def test_generic_create_view(self):
        response = self.client.get(reverse('post:quill-post-create'))
        self.assertEqual(response.status_code, 200)

    def test_generic_detail_view(self):
        quill_post = baker.make(QuillPost)
        response = self.client.get(reverse('post:quill-post-detail', args=[quill_post.id]))
        self.assertEqual(response.status_code, 200)
